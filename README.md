...
yarn install
Настройка окружения
Создайте файл .env в корне проекта (рядом с index.js).
Пример содержимого:
...
Если вы используете Docker‑контейнер для БД (см. ниже), укажите соответствующие параметры.
...
Ошибка подключения к БД – проверьте, что PostgreSQL запущен, а параметры в .env корректны.
